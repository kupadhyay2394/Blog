...
export default function PostDetails() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:8000/api/v1/post/getpostbyID/${id}`, {
          method: "GET",
          headers: {
        ...
        if (!res.ok) {
          throw new Error("Failed to fetch post. It might have been deleted.");
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/v1/post/deletpost/${id}`, {
        method: "DELETE",
  ...
  const handleToggleLike = async () => {
    ...
    // Store the original state to revert in case of an error
    ...
    try {
      const res = await fetch(`https://blog-3-z37i.onrender.com/api/v1/post/toggle-like/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (!res.ok) {
        // If the API call fails, revert the state and throw an error
      ...
      // Also revert state on network or other errors
      setIsLiked(originalIsLiked);
      setLikesCount(originalLikesCount);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4">
        <div className="w-full max-w-lg p-8 text-center bg-white border-t-4 border-red-500 rounded-xl shadow-2xl">
          ...
          <button
            onClick={() => navigate("/")}
            className="px-6 py-2 font-semibold text-white bg-indigo-600 rounded-lg shadow-md hover:bg-indigo-700 transition duration-150"
          >
            Go Back Home
          </button>
        </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <article className="overflow-hidden bg-white border border-gray-100 rounded-2xl shadow-xl">
          ...
          <div className="p-6 md:p-10">
            ...
            {/* Post Meta: Author, Date, and Like Button */}
            <div className="flex flex-wrap items-center justify-between pt-4 pb-8 border-t border-gray-100">
              <div className="flex items-center space-x-4">
                {/* Assuming author has an avatar, otherwise remove img tag */}
                {/* <img className="w-12 h-12 rounded-full" src={post.user.avatarURL || 'default-avatar.png'} alt={post.user.fullName} /> */}
                <div>
              ...
              <div className="flex items-center mt-4 space-x-3 sm:mt-0">
                <button
                  ...
                  className={`p-3 rounded-full transition duration-200 transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 ${isLiked 
                    ? 'bg-red-100 text-red-600 focus:ring-red-500' 
                    : 'bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-500 focus:ring-indigo-500'
                  }`}
        ...
        <div className="mt-12 text-center">
          <button 
            onClick={() => navigate(-1)}
            className="inline-flex items-center px-6 py-3 font-medium text-indigo-600 bg-white border-2 border-indigo-200 rounded-full hover:bg-indigo-50 hover:border-indigo-600 transition duration-300"
          >
